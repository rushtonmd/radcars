...
this.shortUrlRouteControler = RouteController.extend({
	onBeforeAction: function() {
		console.log('Finding shortURL');
		//var x = Cars.findOne({short_url: })
		//CarPages.unsubscribe();
		//CarCurationPages.unsubscribe();
		//CarSearchPages.unsubscribe();
		Router.go('/admin/curation');
...
Router.route('/:source/:_id', function() {
	...
	onAfterAction: function() {
			...
			image = img && img.isUploaded() && img.hasStored("master") && img.url() || "/noimage.jpg";
			...
			SEO.set({
				meta: {
					...
					'twitter:card': 'photo',
					'twitter:site': '@tirekickUS',
					'twitter:title': "TireKick For Sale: " + car.heading,
					'twitter:image': "http://tirekick.us" + image,
					'twitter:url': url
				},
				og: {
					'description': "TireKick For Sale: " + car.heading
